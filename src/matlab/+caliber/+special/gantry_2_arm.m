function [tree, initializer, optimizer] = gantry_2_arm(datafile, angles, mobileIndices, fixedIndices)
% datafile = filename of Bouguet output
%   create this by running Bouguet over the complete set of images
%   selecting the mobile checkerboard, then running it again
%   selecting the fixed checkerboard, then merging the results
...
% mobileIndices: image indices where mobile checkerboard was visible
% fixedIndices: image indices where fixed checkerboard was visible
% see +caliber/+test/twoarm for an example
